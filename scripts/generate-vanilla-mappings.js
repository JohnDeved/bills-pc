...
function createPokemonMapping(pokeemeraldSpecies, pokeapiPokemon) {
  ...
  for (const [internalId, species] of pokeemeraldSpecies) {
    ...
    if (apiPokemon) {
      ...
      mapping[internalId] = {
        name:
          apiPokemon.name.charAt(0).toUpperCase() + apiPokemon.name.slice(1),
        id_name: apiPokemon.name,
        id: apiId,
      };
    } else {
      console.warn(
        `Could not find PokeAPI match for ${species.name} (${internalId})`,
      );
    }
  }
...
function createMoveMapping(pokeemeraldMoves, pokeapiMoves) {
  ...
  for (const [internalId, move] of pokeemeraldMoves) {
    ...
    if (apiMove) {
      ...
      console.warn(
        `Could not find PokeAPI match for ${move.name} (${internalId})`,
      );
    }
  }
...
function createItemMapping(pokeemeraldItems, pokeapiItems) {
  ...
  for (const [internalId, item] of pokeemeraldItems) {
    ...
    if (apiItem) {
      ...
      console.warn(
        `Could not find PokeAPI match for ${item.name} (${internalId})`,
      );
    }
  }
...
async function main() {
  try {
    ...
    const pokemonMapping = createPokemonMapping(
      pokeemeraldSpecies,
      pokeapiPokemon,
    );
