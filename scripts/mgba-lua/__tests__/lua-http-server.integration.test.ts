  ...
  beforeAll(async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const timeout = setTimeout(
        () => reject(new Error("mGBA HTTP server start timeout")),
        15000,
      );
  ...
  describe("WebSocket Functionality", () => {
    it("should handle WebSocket handshake and send welcome message", async () => {
      ...
      expect(welcomeMessage).toBe(
        "Welcome to WebSocket Eval! Send Lua code to execute.",
      );
    ...
    it("should handle WebSocket eval functionality", async () => {
      ...
      const testResult = await new Promise<string>((resolve, reject) => {
        const timeout = setTimeout(
          () => reject(new Error("Eval response timeout")),
          5000,
        );
        ...
        ws.on("message", (data) => {
          ...
          if (
            message.includes("Welcome to WebSocket Eval") &&
            !welcomeReceived
          ) {
