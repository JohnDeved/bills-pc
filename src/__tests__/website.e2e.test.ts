...
test.describe("Pokemon Save Web - E2E Tests", () => {
  ...
  test("should load a save file and display Pokemon data", async ({ page }) => {
    ...
    await page.evaluate(async () => {
      const response = await fetch(
        "/src/lib/parser/__tests__/test_data/emerald.sav",
      );
      const buffer = await response.arrayBuffer();
      const file = new File([buffer], "emerald.sav", {
  ...
  test("should show proper menu options when save data is loaded", async ({
    page,
  }) => {
    // Load save file first
    await loadTestSaveFile(page);
