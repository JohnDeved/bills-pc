...
export const PokemonMoveButton: React.FC<MoveButtonProps> = ({
  ...
  const popoverDirectionClass = opensUpward
    ? "bottom-full mb-1"
    : "top-full mt-1";
  ...
  const baseTypes = usePokemonStore(
    (s) =>
      s.partyList.find((p) => p.id === s.activePokemonId)?.details?.types ?? [],
  );
  const activePokemon = usePokemonStore((s) =>
  ...
  const effectiveTypes =
    megaPreviewEnabled && Array.isArray(megaTypes) && megaTypes.length > 0
      ? megaTypes
      : baseTypes;
  ...
  const effectiveAbilityName = (() => {
    if (
      megaPreviewEnabled &&
      Array.isArray(megaAbilities) &&
      megaAbilities.length > 0
    ) {
      const sorted = [...megaAbilities].toSorted(
    ...
    const sorted = [...abilities].toSorted(
      (a: Ability, b: Ability) => a.slot - b.slot,
    );
    ...
    <div ref={rootRef} className="relative" onWheel={onWheel}>
      ...
      >
        <div className="flex items-center justify-between">
          <span className="text-sm text-foreground truncate w-full block">
            {move.name}
          </span>
        </div>
