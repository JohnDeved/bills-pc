...
export const PokemonStatus: React.FC<PokemonStatusProps> = ({
  ...
  const { megaPreviewEnabled, megaSpriteAniUrl, megaSpritePngUrl } =
    useMegaPreview();
  ...
  return (
    ...
    >
      <PokemonSprite
        ...
        fallbackSrc={
          isActive && megaPreviewEnabled && megaSpritePngUrl
            ? megaSpritePngUrl
            : pokemon.spriteUrl
        }
      ...
      <div className="flex-grow">
        ...
        <div className="w-full bg-background/30 border border-border border-x-2 rounded-sm h-2.5 mt-2 overflow-hidden">
          <div
            className={cn(
              "bg-gradient-to-r h-full transition-all duration-500",
              hpColor,
            )}
