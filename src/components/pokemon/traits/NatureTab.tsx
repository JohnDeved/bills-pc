...
export const NatureTab: React.FC = () => {
  const pokemon = usePokemonStore((s) =>
    s.partyList.find((p) => p.id === s.activePokemonId),
  );
  ...
  function handleSelectCounterpart(index: number) {
    ...
    if (chooseMode === "raise") {
      const newNature = findNatureForEffects(
        index,
        decIndex >= 0 ? decIndex : 1,
      );
      setNature(pokemon.id, newNature);
    } else if (chooseMode === "lower") {
      const newNature = findNatureForEffects(
        incIndex >= 0 ? incIndex : 1,
        index,
      );
      setNature(pokemon.id, newNature);
    }
  ...
  if (natureMods && neutralTotals && currentTotals) {
    ...
    const dec = decIndex;
    if (inc > 0) {
      raisedDelta = Math.max(
        0,
        (currentTotals[inc] ?? 0) - (neutralTotals[inc] ?? 0),
      );
    }
    if (dec > 0) {
      loweredDelta = Math.max(
        0,
        (neutralTotals[dec] ?? 0) - (currentTotals[dec] ?? 0),
      );
    }
  }
  ...
  function getPreviewDelta(index: number): number | null {
    ...
    const delta =
      ((newTotals?.[index] ?? 0) as number) - (currentTotals?.[index] ?? 0);
    return delta;
  }
            ...
            <div className="flex flex-wrap items-center gap-2">
              {incIndex >= 0 && (
                <button
                  type="button"
                  onClick={() =>
                    setChooseMode((prev) => (prev === "raise" ? null : "raise"))
                  }
                ...
                >
                  <IoCaretUp className="text-emerald-300" /> Raises{" "}
                  {getStatAbbr(incIndex)}
                  {raisedDelta > 0 && (
              ...
              {decIndex >= 0 && (
                <button
                  type="button"
                  onClick={() =>
                    setChooseMode((prev) => (prev === "lower" ? null : "lower"))
                  }
                ...
                >
                  <IoCaretDown className="text-rose-300" /> Lowers{" "}
                  {getStatAbbr(decIndex)}
                  {loweredDelta > 0 && (
            ...
            <div className="mt-2">
              {(() => {
                ...
                if (chooseMode === "raise") headerText = "Select stat to Raise";
                  ...
                  headerText = "Select stat to Lower";
                return (
              ...
              <div className="flex flex-wrap gap-1.5">
                {statAbbreviations.map((abbr, i) => {
                  ...
                  if (chooseMode === "raise")
                    interactive = i !== 0 && i !== decIndex;
                  if (chooseMode === "lower")
                    interactive = i !== 0 && i !== incIndex;

                  let ringColor = "hover:ring-ring/30";
                  if (chooseMode === "raise")
                    ringColor = "hover:ring-emerald-300/40";
                  else if (chooseMode === "lower")
                    ringColor = "hover:ring-rose-300/40";
                  ...
                  const padAnimCls =
                    chooseMode && interactive ? "hover:pr-7" : "";
                  return (
                    <div
                      key={`nature-stat-${i}`}
                      className={`${base} ${color} ${interactiveCls} ${padAnimCls}`}
                      onClick={() =>
                        chooseMode && interactive && handleSelectCounterpart(i)
                      }
                      onMouseEnter={() =>
                        chooseMode && interactive
                          ? setHoveredStat(i)
                          : undefined
                      }
                      onMouseLeave={() =>
                        hoveredStat === i ? setHoveredStat(null) : undefined
                      }
                      role={chooseMode ? "button" : undefined}
                      aria-disabled={
                        chooseMode && !interactive ? true : undefined
                      }
                    >
                      {abbr}
                      <span
