...
describe("MgbaWebSocketClient - Memory Watching API", () => {
  ...
  describe("Memory Change Listeners", () => {
    ...
    it("should enforce listener limit", () => {
      ...
      expect(() => client.addMemoryChangeListener(vi.fn())).toThrow(
  ...
  describe("Error Handling", () => {
    ...
    it("should throw error when starting watch without regions", async () => {
      ...
      (
        client as unknown as { connected: boolean; ws: { readyState: number } }
      ).connected = true;
      (
        client as unknown as { connected: boolean; ws: { readyState: number } }
      ).ws = {
      ...
      await expect(client.startWatching([])).rejects.toThrow(
        "No regions to watch",
      );
    });
