...
describe("Pokemon Quetzal Tests", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.log(
        `Ground truth contains ${groundTruth.party_pokemon.length} Pokemon`,
      );
    } catch (error) {
  ...
  describe("Auto-Detection", () => {
    it("should correctly identify Quetzal saves", async () => {
      ...
      expect(autoResult.party_pokemon.length).toBe(
        groundTruth.party_pokemon.length,
      );
  ...
  describe("Save File Parsing", () => {
    ...
    it("should parse correct number of party Pokemon", () => {
      expect(parsedData.party_pokemon).toHaveLength(
        groundTruth.party_pokemon.length,
      );
    });
    ...
    it("should have valid sector mapping", () => {
      ...
      const expectedSectorMap = new Map(
        Object.entries(groundTruth.sector_map).map(([k, v]) => [
          parseInt(k),
          v,
        ]),
      );
    ...
    it("should calculate stats correctly using base stats from API", async () => {
      ...
      const speciesIds = [
        ...new Set(parsedData.party_pokemon.map((p) => p.speciesId)),
      ];
      ...
      if (missing.length > 0) {
        for (const speciesId of missing) {
          try {
            ...
            console.warn(
              `Failed to fetch base stats for species ${speciesId}:`,
              error,
            );
          }
        }
  ...
  describe("EV Writing and Persistence", () => {
    it("should allow writing and reading EVs back correctly", async () => {
      ...
      if (parsedData.party_pokemon.length > 0) {
        ...
        const reconstructed = parser.reconstructSaveFile(
          parsedData.party_pokemon,
        );
        ...
        expect(reparsed.party_pokemon).toHaveLength(
          parsedData.party_pokemon.length,
        );
  ...
  describe("Save File Reconstruction", () => {
    ...
    it("should reflect party changes when reparsing reconstructed data", async () => {
      ...
      expect(
        reparsed.party_pokemon[reparsed.party_pokemon.length - 1]!.speciesId,
      ).toBe(parsed.party_pokemon[0]!.speciesId);
    });
