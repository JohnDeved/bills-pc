...
const PARTY_COLUMNS = [
  {
    label: "Slot",
    width: 5,
    value: (_p: PokemonBase, i: number) => (i + 1).toString(),
  },
  {
    label: "Dex ID",
    width: 8,
    value: (p: PokemonBase) => p.speciesId.toString(),
  },
  ...
  { label: "Lv", width: 4, value: (p: PokemonBase) => p.level.toString() },
  {
    label: "Ability",
    width: 8,
    value: (p: PokemonBase) => p.abilityNumber.toString(),
  },
  { label: "Nature", width: 10, value: (p: PokemonBase) => p.nature },
  {
    label: "Shiny",
    width: 6,
    value: (p: PokemonBase) => p.shinyNumber.toString(),
  },
  {
    label: "HP",
    width: 32,
...
const displayPartyPokemon = (party: readonly PokemonBase[], mode = "FILE") => {
  ...
  party.forEach((p, i) => {
    const row = PARTY_COLUMNS.map((col) =>
      pad(col.value(p, i), col.width),
    ).join("");
    console.log(row);
  });
};
...
const displaySaveblock2Info = (
  { player_name, play_time }: SaveData,
  mode = "FILE",
) => {
  console.log(`\n--- SaveBlock2 Data (${mode} MODE) ---`);
  console.log(`Player Name: ${player_name}`);
  console.log(
    `Play Time: ${play_time.hours}h ${play_time.minutes}m ${play_time.seconds}s`,
  );
};
...
const displayPartyPokemonRaw = (party: readonly PokemonBase[]) => {
  ...
  party.forEach((p, i) => {
    console.log(`\n--- Slot ${i + 1}: ${p.nickname} ---`);
    console.log(
      [...p.rawBytes].map((b) => b.toString(16).padStart(2, "0")).join(" "),
    );
  });
};
  ...
  while (pos < raw.length) {
    ...
    for (let i = 0; i < lineBytes.length; ) {
      ...
      if (field && idx === field[0]) {
        ...
        const shortName =
          n.length > width ? `${n.slice(0, Math.max(0, width - 1))}.` : n;
...
async function parseAndDisplay(
  ...
  if (typeof input === "string") {
    ...
    if (!options.skipDisplay) {
      console.log(
        `🎮 Connected to: ${parser.gameConfig?.name ?? "unknown"} (via mGBA WebSocket)`,
      );
    }
  }
...
async function watchModeFile(
  ...
  console.log(
    `🔄 Starting file watch mode (updating every ${options.interval}ms)...`,
  );
...
async function main() {
  ...
  if (websocket) {
    ...
    // File mode
    const savePath = argv.find(
      (arg) => arg.match(/\.sav$/i) && fs.existsSync(path.resolve(arg)),
    );
    if (!savePath) {
