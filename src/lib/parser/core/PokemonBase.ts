...
export class PokemonBase {
  ...
  constructor(
    ...
    const pokemonSize =
      typeof config.pokemonSize === "number" ? config.pokemonSize : 100;
  ...
  protected setEncryptedSubstruct(
    substructIndex: number,
    ...
    if (decryptedData.length !== 12) {
      throw new Error(
        `Substruct data must be 12 bytes, got ${decryptedData.length}`,
      );
    }
    ...
    for (let i = 0; i < 12; i += 4) {
      ...
      const origView = new DataView(
        this.data.buffer,
        this.data.byteOffset + substructOffset + i,
        4,
      );
  ...
  protected getDecryptedSubstruct(
    data: Uint8Array,
    substructIndex: number,
  ): Uint8Array {
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    ...
    const encryptedData = new Uint8Array(
      data.buffer,
      data.byteOffset + substructOffset,
      12,
    );
  ...
  get speciesId() {
    ...
    const subView = new DataView(
      substruct0.buffer,
      substruct0.byteOffset,
      substruct0.byteLength,
    );
  ...
  get move1() {
    ...
    const subView = new DataView(
      substruct1.buffer,
      substruct1.byteOffset,
      substruct1.byteLength,
    );
  ...
  get ivs(): readonly number[] {
    ...
    const subView = new DataView(
      substruct3.buffer,
      substruct3.byteOffset,
      substruct3.byteLength,
    );
  ...
  get isShiny(): boolean {
    if (this.config.isShiny)
      return this.config.isShiny(this.personality, this.otId);
  ...
  get shinyNumber(): number {
    if (this.config.getShinyValue)
      return this.config.getShinyValue(this.personality, this.otId);
  ...
  get isRadiant(): boolean {
    if (this.config.isRadiant)
      return this.config.isRadiant(this.personality, this.otId);
  ...
  get nature(): string {
    ...
    return (
      this.config.calculateNature?.(this.personality) ??
      natures[this.personality % 25]!
    );
  }
  ...
  get natureModifiersString(): { increased: string; decreased: string } {
    ...
    return {
      increased:
        increased >= 0 ? (statStrings[increased] ?? "Unknown") : "None",
      decreased:
        decreased >= 0 ? (statStrings[decreased] ?? "Unknown") : "None",
    };
  }
