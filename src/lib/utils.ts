...
const buildPokemonBlock = (pokemon: UIPokemonData, index: number): string => {
  const speciesName =
    formatIdName(pokemon.data.nameId) ?? `Species ${pokemon.data.speciesId}`;
  const nickname = pokemon.data.nickname || speciesName;
  const abilitySlot = pokemon.data.abilityNumber + 1;
  const ability =
    pokemon.details?.abilities?.find((entry) => entry.slot === abilitySlot)
      ?.name ??
    pokemon.details?.abilities?.[0]?.name ??
    `Ability Slot ${abilitySlot}`;
  const itemName =
    pokemon.details?.item?.name ??
    formatIdName(pokemon.data.itemIdName) ??
    "None";
...
export const buildTeamClipboardText = (
  ...
  const blocks = party.map((pokemon, index) =>
    buildPokemonBlock(pokemon, index),
  );
  return `${headerTitle}\n${underline}\n\n${blocks.join("\n\n")}`;
};
