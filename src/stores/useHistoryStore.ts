...
function areSnapshotsEqual(a: HistorySnapshot, b: HistorySnapshot): boolean {
  ...
  if (bytesA.length !== bytesB.length || idsA.length !== idsB.length)
    return false;
...
function getCurrentSnapshot(
  ...
  try {
    ...
    const idsBySlot =
      idsBySlotOverride ??
      usePokemonStore.getState().partyList.map((p) => p.id);
...
export const useHistoryStore = create<HistoryStore>((set, get) => ({
  ...
  undo: async () => {
    ...
    try {
      ...
      await parse(previous.bytes, { transient: true });
    } finally {
  ...
  redo: async () => {
    ...
    try {
      ...
      await parse(next.bytes, { transient: true });
    } finally {
      set({ isApplying: false });
    }
  },
  ...
  reset: async () => {
    ...
    try {
      ...
      await parse(initial.bytes, { transient: true });
